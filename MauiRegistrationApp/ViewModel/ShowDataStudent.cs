...
namespace MauiRegistrationApp.ViewModel
{
    public partial class ShowDataStudent : ObservableObject
    {
        ...
        public async Task CheckLogin()
        {
            ...
            try
            {
                var userData = await ReadJsonAsync();
                ...
                bool isAuthenticated = userData.Student.Email == trimmedEmail && 
                                    userData.Student.Password == trimmedPassword;

                if (isAuthenticated)
                {
                    User = userData; // Set the user data
                    await LoadDataAsync(); // Load remaining data
        ...
        [RelayCommand]
        public async void RegisterCourse(AvailableCourse course)
        {
           bool confirm = await  Application.Current.MainPage.DisplayAlert(
                "ยืนยันลงทะเบียนวิชา",
                $"คุณต้องการลงทะเบียนจริงหรือไม่?",
                "ตกลง",
                "ยกเลิก");
